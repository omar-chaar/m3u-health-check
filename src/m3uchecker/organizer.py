            ...
            if result:
                for item in result:
                    ...
                    if channel_id is not None:
                        # Map back to original index in full list
...
def build_extinf_line(channel, new_name, new_group):
    """Build a new EXTINF line with updated name and group."""
    # Start with base attributes
    attrs = []

    # Duration (always -1 for live streams)
    duration = "-1"
...
def write_organized_m3u(filename, channels, reorganization_map):
    """Write organized channels to M3U file, sorted by group then name."""
    filepath = os.path.join(OUTPUT_DIR, filename)

    # Build list of (group, name, channel, extinf_line) tuples
    organized = []
    for idx, ch in enumerate(channels):
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        for group, name, ch, extinf_line in organized:
            # Add group comment for readability (optional visual separator)
...
def get_output_filename():
    """Get output filename for organized playlist."""
